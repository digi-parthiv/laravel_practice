<x-default-layout>
    ...
    @section('breadcrumbs')
        {{ Breadcrumbs::render('e-commerce.shipment.index') }}
    @endsection
    ...
    @push('scripts')
        ...
        <script>
            document.getElementById('mySearchInput').addEventListener('keyup', function () {
                window.LaravelDataTables['shipment-table'].search(this.value).draw();
            });
            document.addEventListener('livewire:init', function () {
                Livewire.on('success', function () {
                    $('#kt_modal_add_user').modal('hide');
                    window.LaravelDataTables['shipment-table'].ajax.reload();
                });
            });
